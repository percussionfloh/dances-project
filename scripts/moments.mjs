...
const momentYamlPath = `${__dirname}/../content/raw-data/raw-moment.yaml`;
const momentContentYamlPath = `${__dirname}/../content/data/moment.yaml`;
...
const momentYaml = yaml.load(fs.readFileSync(momentYamlPath, 'utf8'));
...
function parseTimepoint(tp) {
    ...
    return {
        ...
        beat: convertFloatToTwoPart(parseFloat(beatStr)),
    };
}
...
function getParsedMomentPart(momentPart, kernLines, data) {
    const newProperties = {
        startLine: null,
        endLine: null,
        startKey: null,
    ...
    const startPoint = momentPart.start;
    const endPoint = momentPart.end;
    ...
    if (Array.isArray(momentPart.children)) {
        momentPart.children.forEach(child => {
            const parsed = getParsedMomentPart(child, kernLines, data);
            parsedChildren.push(parsed);
        });
    }

    return Object.assign({}, momentPart, newProperties, {
...
for (const pathToKernScoreObj of pathToKernScores) {
    getFiles(pathToKernScoreObj.path).forEach(file => {
        ...
        const pieceMoment = momentYaml[id];

        if (!pieceMoment || pieceMoment.length === 0) {
            console.warn(`❌ No Special Moment found for ${id}`);
            return;
        }
        
        console.log(`✅ Special Moment found for ${id}`);
...
*/
        ...
        pieceMoment.forEach((momentPart) => {
            const parsedMomentPart = getParsedMomentPart(momentPart, kernLines, data);
            momentData[id].push(parsedMomentPart)
        });
    });
}
